...
function IndexDone(props: IndexPreventivesProps) {
    const NameIndex = 'done';
    const EmptyTextIndex = 'Nenhuma preventiva foi feita neste ano';
                    ...
                    {preventives?.map(preventive => 
                        <li key={preventive.id + preventive.code}>
                            <BoxIndexPreventives 
                                ...
                                alertTitle="Deletar Preventiva"
                                alertText={"Deseja deletar esta preventiva de: " + handleDateIndex(preventive.preventives[0].preventiveDate) + " do Computador: " + preventive.code + "?"}
                                refresh={refresh}
                                deletePreventive={() => deletePreventive(preventive.preventives[0].id, preventive.id)}
                                setErrors={setError}
                            />
                        </li>    
                    )}
                    ...
                    {preventives?.map(preventive => 
                        <li key={preventive.id + preventive.code}>
                            <BoxIndexPreventives 
                                ...
                                equipament={preventive}
                                alertTitle="Deletar Preventiva"
                                alertText={"Deseja deletar esta preventiva de: " + handleDateIndex(preventive.preventives[0].preventiveDate) + " do Nobreak: " + preventive.code + "?"}
                                refresh={refresh}
                                deletePreventive={() => deletePreventive(preventive.preventives[0].id, preventive.id)}
                                setErrors={setError}
                            />
                    ...
                    {preventives?.map(preventive => 
                        <li key={preventive.id + preventive.code}>
                            <BoxIndexPreventives 
                                ...
                                equipament={preventive}
                                alertTitle="Deletar Preventiva"
                                alertText={"Deseja deletar esta preventiva de: " + handleDateIndex(preventive.preventives[0].preventiveDate) + " do DVR: " + preventive.code + "?"}
                                refresh={refresh}
                                deletePreventive={() => deletePreventive(preventive.preventives[0].id, preventive.id)}
                                setErrors={setError}
                            />
